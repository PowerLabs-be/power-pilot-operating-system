...
def test_init(shell, shell_json):
    ...
    while True:
        supervisor_info = "\n".join(
            shell.run_check("ha supervisor info --no-progress --raw-json || true")
        )
        # make sure not to fail when Supervisor is restarting
        supervisor_info = (
            json.loads(supervisor_info) if supervisor_info.startswith("{") else None
        )
        ...
        if (
            supervisor_data
            and supervisor_data["version"] == supervisor_data["version_latest"]
        ):
...
def test_os_update(shell, shell_json, target):
    ...
    stable_version = shell_json(
        "curl -sSL https://raw.githubusercontent.com/PowerLabs-be/power-pilot-version/refs/heads/master/stable.json"
    )["ppos"]["ova"]
    ...
    while True:
        output = "\n".join(
            shell.run_check(
                f"ha os update --no-progress --version {stable_version} || true",
                timeout=120,
            )
        )
    ...
    while True:
        if check_container_running("hassio_supervisor") and check_container_running(
            "hassio_cli"
        ):
            break
    ...
    assert os_info["data"]["version"] == stable_version, (
        "OS did not update successfully"
    )
